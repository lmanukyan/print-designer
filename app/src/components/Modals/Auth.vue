                ...
                <q-tab-panel name="login">
                    ...
                    >
                      ...
                      <q-input
                        ...
                        :disable="loading"
                        :rules="[
                          (val) => (val && val.length > 5) || 'Пароль должен содержать не менее 8 символов․',
                        ]"
                      />
                      <div class="q-ml-none">
                        <q-btn :loading="loading" label="Войти" type="submit" color="primary"/>
                      </div>
                    </q-form>
                ...
                <q-tab-panel name="register">
                  ...
                  >
                    <q-input
                      ...
                      label="Ваше имя"
                      lazy-rules
                      :rules="[ (val) => (val && val.length > 0) || 'Имя не может быть пустым.' ]"
                    />
...
export default {
  ...
  methods: {  
    ...
    async onRegsiter(){
      this.loading = true;
      let user = await UserService.register(this.name, this.email, this.password)
      if(user){
        this.setUser(user);
        this.setAuthModalOpened(false);
      }
